...
silently mix two different things. Node count and wall time are averaged over every
run, since both are spent whether or not a path is found. Where a statistic is
undefined, for instance the cost of a planner that never succeeded, it is reported as
``nan`` rather than replaced by a placeholder.
"""
